...
public interface ModuleRepository extends JpaRepository<Module, Long> {
    ...
    @Query("select md from Module md left join fetch md.subModules smd left join fetch smd.menus mn" +
        " left join fetch mn.actions ac left join ac.users u where ac in :actions or u.id = :userId and md.parentModule is null")
    List<Module> findAllModuleByUserId2(@Param("userId") Long userId, @Param("actions") Collection<Action> actions);
